...
check("wizard step 2 honest formats", ED.views.wizard("2"), ["CSV · XLSX · PDF", "no OCR", "sample CSV", "No result files yet"], ["not yet parsed", "isn’t built yet"]);
...
ws.resultFiles = [
  ...
  { name: "8C-results.pdf", status: "error", statusText: "No question-results table could be read from this PDF. Exam Detective reads text-based item-analysis layouts with rows like “12 … 84%”." }
];
ws.key = ["A", "B", "C", "D", "A", "B", "C", "D", "A", "B"];
ws.keyCount = 10;
ED.wizard.setState(ws);
check("wizard step 2 (uploaded)", ED.views.wizard("2"), ["8B-results.csv", "Parsed — 12 students", "No question-results table"], ["isn’t built yet"]);
check("wizard step 5 (uploaded)", ED.views.wizard("5"), ["Uploaded Data", "12 students", "couldn’t be read"], ["129", "Possible key mismatch", "isn’t built yet"]);
...
// all uploads failed -> per-file reasons, never "isn't built yet"
var wsBlocked = ED.wizard.getState();
var keepFiles = wsBlocked.resultFiles;
wsBlocked.resultFiles = [
  { name: "8E 2026.pdf", status: "error", statusText: "This PDF appears to be a scan (images of pages, no machine-readable text). Exam Detective doesn’t do OCR yet." },
  { name: "8C 2026.pdf", status: "error", statusText: "No question-results table could be read from this PDF." }
];
ED.wizard.setState(wsBlocked);
check("wizard step 5 (all files failed)", ED.views.wizard("5"), [
  "8E 2026.pdf", "appears to be a scan", "8C 2026.pdf", "No question-results table", "parsed for real"
], ["isn’t built yet", "Export your results as <b>CSV</b>"]);
check("wizard step 6 (all files failed)", ED.views.wizard("6"), ["Can’t run yet", "8E 2026.pdf"], ["isn’t built yet"]);
wsBlocked.resultFiles = keepFiles;
ED.wizard.setState(wsBlocked);
